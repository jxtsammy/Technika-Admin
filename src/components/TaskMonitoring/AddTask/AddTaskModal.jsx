...
export default function AddTaskModal({ isOpen, onClose, onCreateTask }) {
    ...
    const [mapPosition, setMapPosition] = useState([5.6037, -0.187]);
    ...
    useEffect(() => {
        function handleClickOutside(event) {
            if (
                ...
                setShowCompanyTips(false);
            if (
                locationRef.current &&
                !locationRef.current.contains(event.target)
            )
                setShowLocationTips(false);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const created = await tasksApi.create({
                ...
                // Task.location schema expects separate latitude/longitude fields,
                // not a coordinates array — sending the wrong shape here meant
                // Mongoose was silently dropping the geocoded position on every
                // task, even though the map above was finding it correctly.
            ...
            <div className="split-panel-modal-container">
                ...
                <div className="modal-right-form-panel">
                    ...
                    <form
                        ...
                        className="modal-compact-form-layout"
                    >
                        ...
                        <div className="form-two-column-grid-row">
                            <div
                                className="form-input-field-block custom-relative"
                                ref={companyRef}
                            >
                        ...
                        <div className="form-two-column-grid-row">
                            <div className="form-input-field-block">
                                ...
                                <div className="custom-select-input-wrapper">
                                    <span
                                        className={`priority-indicator-dot ${priority.toLowerCase()}`}
                                    ></span>
                                    <select
                                        id="priority"
                                        value={priority}
                                        ...
                                        {suggestedTechnicians.length > 0 ? (
                                            suggestedTechnicians.map((t) => (
                                                <li
                                                    key={t.id}
                                                    onClick={() => {
                                                        setTechnician(t.name);
                                                        setTechnicianId(t.id);
                                                        setShowTechTips(false);
                                                    }}
                                                >
                                                    {t.name}{" "}
                                                    <span className="tech-region-tag">
                                                        (
                                                        {t.isOnline
                                                            ? "Online"
                                                            : "Offline"}
                                            ...
                                            <li className="no-suggestions-indicator-item">
                        ...
                        <div className="form-two-column-grid-row">
                            ...
                            <div className="form-input-field-block">
                                <label htmlFor="contactEmail">
                                    Contact Email (for completion code)
                                </label>
                                <input
                                    id="contactEmail"
                                    type="email"
                                    placeholder="onsite.contact@company.com"
                                    value={contactEmail}
                                    onChange={(e) =>
                        ...
                        <footer className="modal-panel-footer-row">
                            ...
                            <button
                                type="button"
                                className="wizard-back-navigation-btn"
                                onClick={onClose}
                                disabled={submitting}
                            >
                                Back
                            ...
                            <button
                                type="submit"
                                className="wizard-create-task-submit-btn"
                                disabled={submitting}
                            >
